...
function DataTable({ rows }) {
  if (!rows.length)
    return <p className="text-center text-gray-400 mt-6">Aucune donnée disponible pour cette sélection.</p>;

  return (
    <table className="w-full border-separate border-spacing-2 my-4">
      <thead>
        <tr>
          {["N°","Station","Latitude","Longitude","DateTime","TIDE HEIGHT","WIND SPEED","WIND DIR","AIR PRESSURE","AIR TEMPERATURE","DEWPOINT","HUMIDITY"].map((h) => (
            <th key={h} className="border border-slate-600 rounded-md">{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
